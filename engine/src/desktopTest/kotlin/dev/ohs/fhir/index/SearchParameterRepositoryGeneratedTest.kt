 ...
 * asserts that the codegen output matches what the spec says for every kotlin-fhir resource type.
 *
 * Both the codegen and the test consult the same source file, so this is not an independent oracle
 * the way the original android-fhir test (which read HAPI's `@SearchParamDefinition` annotations)
 * was. It still catches codegen bugs in the code-emission stage — the two paths through the data
 * are different — but it cannot catch a bug shared by both spec readers.
 */
